...
## 库代码如下:
``` js
import React, { Context, FC, createContext, useContext, useReducer, Dispatch, Reducer } from 'react';
/** 
 ...
 *  @genericity 类型R 定义reducer的类型,可选参数
 *  @genericity 类型T state的类型,可选参数
 */
export default (reducer: Function, initialState: any, init?: any) => (Com: FC<any>) => {
        return () => {
 ...
 *  @T 接收一个state的类型，是总体的state的类型,可选参数
 */
